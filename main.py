...
import time
from sprite import *
from settings import WIDTH, HEIGHT, title, GAME_SIZE, TILESIZE, FPS, BGCOLOUR, GREEN, BLACK
...
class Game:
    ...
    def new(self):
        ...
        self.start_game = False
        self.buttons_list = []
        self.buttons_list.append(Button(500, 100, 200, 50, "Shuffle", GREEN, BLACK))
        self.buttons_list.append(Button(500, 170, 200, 50, "Reset", GREEN, BLACK))
        self.draw_tiles()
    ...
    def update(self):
        if self.start_game:
            if self.tiles_grid == self.tiles_grid_completed:
                ...
                self.save_score()
                self.display_win_image()  # Display the win text
    ...
    def draw_grid(self):
        for row in range(-1, GAME_SIZE * TILESIZE, TILESIZE):
            pygame.draw.line(self.screen, BLACK, (row, 0), (row, GAME_SIZE * TILESIZE))
        for col in range(-1, GAME_SIZE * TILESIZE, TILESIZE):
            pygame.draw.line(self.screen, BLACK, (0, col), (GAME_SIZE * TILESIZE, col))
    ...
    def display_win_image(self):
        ...
        pygame.display.flip()
        pygame.time.wait(3000)  # Display the text for 3 seconds

    def display_reset_message(self):
        font = pygame.font.Font(None, 74)
        text = font.render("RESETTING...", True, (255, 0, 0))
        text_rect = text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
        self.screen.blit(text, text_rect)
        pygame.display.flip()
        pygame.time.wait(1000)  # Display the text for 1 second
